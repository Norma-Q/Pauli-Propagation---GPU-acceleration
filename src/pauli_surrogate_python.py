...
# src/pauli_surrogate_python.py 내 PauliRotation 클래스 수정
...
class PauliRotation(Gate):
    ...
    - param_idx: parameter index for surrogate (VQC용)
    - embedding_idx: data embedding index (Prior 데이터용) [추가]
    """
    pauli: str  
    qubits: List[int]
    param_idx: int = -1
    embedding_idx: int = -1  # 데이터 임베딩을 위한 인덱스 추가
    
    def __repr__(self):
        # 구분을 위해 repr도 수정하면 디버깅이 편합니다.
        if self.embedding_idx >= 0:
            return f"R{self.pauli}{self.qubits}(prior[{self.embedding_idx}])"
